...
public boolean isRessources(Employe employe) {
        connection = getConnection();
        try {
            pst = connection.prepareStatement("SELECT * FROM employe WHERE employePrenom=? AND employeMatricule=? AND employeCategorie=5");
            pst.setString(1, employe.getEmployePrenom());
            pst.setString(2, employe.getEmployeMatricule());
            res = pst.executeQuery();
            while (res.next()) {
...
public boolean isRessources(String prenom) {
        connection = getConnection();
        try {
            pst = connection.prepareStatement("SELECT * FROM employe WHERE employePrenom=? AND employeCategorie=5");
